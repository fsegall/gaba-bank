...
function resolveConnString(): string {
  ...
  try {
    const u = new URL(raw)
    const looksDockerHost = (h: string) =>
      h === 'db' || h === 'postgres' || h === 'postgresql'
...
export async function tx<T>(fn: (c: PoolClient) => Promise<T>, client?: PoolClient): Promise<T> {
  if (client) {
    const sp = `sp_${Date.now()}_${Math.floor(Math.random()*1e6)}`
    await client.query(`SAVEPOINT ${sp}`)
    try { const v = await fn(client); await client.query(`RELEASE SAVEPOINT ${sp}`); return v }
