...
// ---------- Tipos ----------
type Sep10Challenge = { transaction: string; network_passphrase?: string };
type Sep10Token = { token: string; issued_at?: string; expires_at?: string };
type Sep38QuoteReq = {
  sell_asset: string;
  buy_asset: string;
  sell_amount?: string;
...
const {
  ...
  ANCHOR_QUOTE_URL,
  STELLAR_TREASURY_PUBLIC,
  STELLAR_TREASURY_SECRET,
  STELLAR_TREASURY_SECRET_FILE,
  STELLAR_NETWORK,
} = process.env;
...
// ---------- Utils ----------
function readSecretFromEnvOrFile(value?: string, filePath?: string): string {
...
export async function resolveEndpoints(force = false): Promise<Endpoints> {
  if (endpoints && !force) return endpoints;
  ...
  const { data } = await httpGet<string>(ANCHOR_TOML_URL, {}, 10_000);
  const t: any = toml.parse(data);
...
export async function sep10_start(account = TREASURY_PUB) {
  ...
  if (!eps.WEB_AUTH_ENDPOINT) throw new Error("SEP-10 not configured on anchor");
  const { data } = await httpGet<Sep10Challenge>(
    `${eps.WEB_AUTH_ENDPOINT}?account=${encodeURIComponent(account)}`,
    {},
    10_000
  );
  return data;
...
export async function sep10_token(challenge: Sep10Challenge) {
  const eps = await resolveEndpoints();
  if (!eps.WEB_AUTH_ENDPOINT) throw new Error("SEP-10 not configured on anchor");

  const secret = readSecretFromEnvOrFile(
    STELLAR_TREASURY_SECRET,
    STELLAR_TREASURY_SECRET_FILE
  );
  if (!secret) throw new Error("Missing STELLAR_TREASURY_SECRET (or *_FILE)");
  ...
  const { data } = await httpPost<Sep10Token>(
    eps.WEB_AUTH_ENDPOINT,
...
// ---------- SEP-38 ----------
export async function sep38_quote(jwt: string, req: Sep38QuoteReq) {
  const eps = await resolveEndpoints();
  if (!eps.SEP38_QUOTE_SERVER) throw new Error("SEP-38 not configured on anchor");
  const { data } = await httpPost<Sep38QuoteRes>(
    `${eps.SEP38_QUOTE_SERVER}/quote`,
    req,
...
// ---------- SEP-24 ----------
export async function sep24_deposit(jwt: string, asset_code: string, account: string) {
  ...
  const { data } = await httpPost<{ id: string; url: string }>(
...
// ---------- SEP-12 ----------
export async function sep12_putCustomer(jwt: string, payload: Record<string, any>) {
  const eps = await resolveEndpoints();
  if (!eps.KYC_SERVER_URL) throw new Error("SEP-12 not configured on anchor");
  const { data } = await httpPut(
    `${eps.KYC_SERVER_URL}/customer`,
    payload,
...
export async function sep12_getCustomer(jwt: string, account: string) {
  const eps = await resolveEndpoints();
  if (!eps.KYC_SERVER_URL) throw new Error("SEP-12 not configured on anchor");
