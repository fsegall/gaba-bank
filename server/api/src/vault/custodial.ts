...
export type VaultRow = {
  ...
  base_asset_contract_id: string; base_asset_decimals: number;
  manager_g: string; treasury_g: string; fee_bps: number;
};

export async function getUSDCBalanceUnits(treasuryG: string, usdcC: string): Promise<bigint> {
...
export async function getVault(vaultId: string): Promise<VaultRow> {
  const { rows } = await pool.query('select * from vaults where id=$1', [vaultId]);
  if (!rows[0]) throw new Error('vault not found');
  return rows[0];
}

export async function getTotals(vaultId: string) {
...
export function pricePerShare(navUnits: bigint, totalShares: bigint, shareDecimals = 9): { num: bigint, den: bigint } {
...
export async function deposit(vaultId: string, userId: string, amountUnits: bigint) {
  ...
  const fee = (amountUnits * BigInt(v.fee_bps)) / 10_000n;
...
async function currentNavUnits(vaultId: string): Promise<bigint> {
