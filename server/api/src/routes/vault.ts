...
// --- helpers ---
function parseHumanToUnits(v: string, d = USDC_DECIMALS): bigint {
  const [i, f = ""] = String(v).trim().split(".");
  const pad = (f + "0".repeat(d)).slice(0, d);
  return BigInt((i || "0") + pad);
}

async function buildInvokeXdr(sourceG: string, contractId: string, fn: string, args: any[]) {
...
r.post("/vault/tx/deposit", async (req, res, next) => {
  try {
    ...
    if (!fromG || !amount) return res.status(400).json({ error: "fromG and amount are required" });
    ...
    const xdr = await buildInvokeXdr(
      ...
      [
        ...
        nativeToScVal(amountUnits.toString(), { type: "i128" } as any),
      ],
    );
...
r.post("/vault/tx/withdraw", async (req, res, next) => {
  try {
    ...
    if (!ownerG || !shares) return res.status(400).json({ error: "ownerG and shares are required" });
    ...
    const xdr = await buildInvokeXdr(
      ...
      [
        ...
        nativeToScVal(sharesUnits.toString(), { type: "i128" } as any),
      ],
    );
    res.json({ xdr, network: PASSPHRASE });
  } catch (e) { next(e); }
...
async function viewI128(fn: string): Promise<string> {
...
r.get("/vault/events", async (req, res, next) => {
  try {
    const { user_id, limit = 50 } = req.query as any;
    const params: any[] = [];
    let sql = `SELECT kind, amount, shares, tx_hash, created_at FROM vault_events`;
    if (user_id) { sql += ` WHERE user_id = $1`; params.push(user_id); }
...
r.get("/vault/position", async (req, res, next) => {
  try {
    const { user_id, vault_address } = req.query as any;
    if (!user_id || !vault_address) return res.status(400).json({ error: "user_id and vault_address are required" });
    const q = await pg.query(
      `SELECT shares FROM vault_positions WHERE user_id=$1 AND vault_address=$2`,
...
r.post("/vault/log", async (req, res, next) => {
  try {
    ...
    if (!user_id || !vault_address || !kind) return res.status(400).json({ error: "missing fields" });
    await pg.query(
      `INSERT INTO vault_events (user_id, vault_address, kind, amount, shares, tx_hash, payload)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [user_id, vault_address, kind, amount ?? null, shares ?? null, tx_hash ?? null, payload ?? null],
    );
    res.json({ ok: true });
  } catch (e) { next(e); }
});
