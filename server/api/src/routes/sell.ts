...
import type { ExecFill } from "../services/execProvider/types.js";
import { tracer } from '../observability/otel.js'
import {
  ordersCreatedTotal,
  ordersFilledTotal,
  chunkSellExecutedTotal,
  chunkSellDurationSeconds,
...
const WALLET_COL =
  process.env.WALLET_BALANCE_COL === "balance_units" ? "balance_units" : "balance";
...
// helper
function chunkUnits(total: bigint, n: number): bigint[] {
  const base = total / BigInt(n);
  const rem = total % BigInt(n);
  const arr = Array(n).fill(base);
  for (let i = 0; i < Number(rem); i++) arr[i] = arr[i] + 1n;
  return arr;
}

// helper para manter consistência de toString()
function totalFilledBrlUnitsStr(v: bigint) {
  return v.toString();
}
...
router.post("/sell", async (req: Request, res: Response) => {
  ...
  if (amount_units <= 0n) return res.status(400).json({ error: "amount must be > 0" });
  ...
  try {
    ...
    if (client_ref) {
      const { rows } = await client.query(
        `SELECT id FROM orders WHERE user_id = $1 AND client_ref = $2 AND side = 'SELL' LIMIT 1`,
        [userId, client_ref]
      );
      if (rows[0]) {
        const { rows: ord } = await client.query(
          `SELECT o.*, COALESCE(json_agg(t.*) FILTER (WHERE t.id IS NOT NULL), '[]') AS trades
           FROM orders o
    ...
    const { rows: wAssetRows } = await client.query(
      `SELECT user_id, asset, ${WALLET_COL} AS bal
       FROM wallets
       WHERE user_id = $1 AND asset = $2
       FOR UPDATE`,
      [userId, symbol]
    );
    if (!wAssetRows[0]) throw new Error(`Wallet for ${symbol} not found`);

    const { rows: wBrlRows } = await client.query(
      `SELECT user_id, asset, ${WALLET_COL} AS bal
       FROM wallets
    ...
    if (!wBrlRows[0]) throw new Error("BRL wallet not found");

    const wAsset = wAssetRows[0];
    const wBrl = wBrlRows[0];

    // Saldo suficiente?
    const currentAssetUnits: bigint = USE_UNITS
      ? BigInt(wAsset.bal)
      : BigInt(Math.floor(Number(wAsset.bal) * Math.pow(10, decimals)));

    if (currentAssetUnits < amount_units) throw new Error("Insufficient asset balance");

    // Cria ordem
    const { rows: orderRows } = await client.query(
      `INSERT INTO orders (user_id, symbol, side, requested_units, client_ref, status, amount_brl_centavos, filled_brl_centavos, received_brl_centavos, filled_units, avg_price_brl_x100)
       VALUES ($1, $2, 'SELL', $3, $4, 'NEW', 0, 0, 0, 0, 0)
       RETURNING *`,
      [userId, symbol, amount_units.toString(), client_ref || null]
    );
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      try {
        ...
        const fill: ExecFill = await exec.sell({ symbol, amount_units: partUnits });
        ...
        if (!isDuplicate) {
          if (USE_UNITS) {
            ...
            await client.query(
              `UPDATE wallets SET ${WALLET_COL} = ${WALLET_COL} - $1 WHERE user_id = $2 AND asset = $3`,
              [partUnits.toString(), wAsset.user_id, wAsset.asset]
            );
            await client.query(
              `UPDATE wallets SET ${WALLET_COL} = ${WALLET_COL} + $1 WHERE user_id = $2 AND asset = 'BRL'`,
              [tradeIns.rows[0].received_brl_centavos, wBrl.user_id]
            );
          } else {
            ...
            await client.query(
              `UPDATE wallets SET ${WALLET_COL} = ${WALLET_COL} - $1 WHERE user_id = $2 AND asset = $3`,
              [partUnitsHuman, wAsset.user_id, wAsset.asset]
            );
            await client.query(
              `UPDATE wallets SET ${WALLET_COL} = ${WALLET_COL} + $1 WHERE user_id = $2 AND asset = 'BRL'`,
              [receivedBrlHuman, wBrl.user_id]
            );
          }
        ...
        span.setAttribute('error', true as any);
        throw e;
      } finally {
        endTimer();
        span.end();
    ...
    const avgPriceBrlX100 =
      totalFilledUnits > 0n ? (totalReceivedBrl * 10_000n) / totalFilledUnits : 0n;
    ...
    await client.query(
      `UPDATE orders
       SET status = 'FILLED',
           ...
           last_filled_at = NOW()
       WHERE id = $4`,
      [totalFilledBrlUnitsStr(totalFilledUnits), totalReceivedBrl.toString(), avgPriceBrlX100.toString(), order.id]
    );
