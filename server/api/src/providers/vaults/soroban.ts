...
import {
  ...
  async function prepareAndSend(tx: any): Promise<{ hash: string }> {
    ...
    if (sendResp.status === 'ERROR') throw new Error(`Soroban send error: ${sendResp.errorResult}`);
    ...
    for (;;) {
      ...
      if (r.status === 'FAILED') throw new Error(`tx failed: ${hash}`);
      await new Promise(r => setTimeout(r, 800));
    }
  }
  
  async function callReadonly<T = unknown>(contract: Contract, method: string, ...args: xdr.ScVal[]): Promise<T> {
  ...
  async function invokeContract(method: string, ...args: xdr.ScVal[]): Promise<{ hash: string }> {
    const source = await loadAccount(treasuryPub);
  ...
  export class SorobanVaultProvider implements IVaultProvider {
    kind: 'soroban' = 'soroban';
    ...
    private async readSharesOf(addr: string): Promise<bigint> {
      const c = new Contract(vaultId);
      // Assumindo: shares_of(addr: Address) -> i128
      const out = await callReadonly<string>(c, 'shares_of', Address.fromString(addr).toScVal());
      return BigInt(out);
    }
  
    // Lê saldo subjacente (USDC) do endereço (view)
    private async readUnderlyingOf(addr: string): Promise<bigint> {
      const c = new Contract(vaultId);
      // Assumindo: underlying_of(addr: Address) -> i128 (base units do USDC)
      const out = await callReadonly<string>(c, 'underlying_of', Address.fromString(addr).toScVal());
      return BigInt(out);
    }
  
    // (Opcional) Aprovação de token padrão p/ o vault “pullar” fundos
    private async approve(tokenContractId: string, fromG: string, spenderC: string, amount: bigint) {
      ...
      const { hash } = await invokeContract.call(null as any, 'approve', from, spender, amt, exp); // reuse helper
      return hash;
    }
  }
