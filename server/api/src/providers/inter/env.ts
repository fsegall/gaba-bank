...
export type InterResolved = {
  ...
  caPath?: string
  scopes: {
    COB: string        // emitir/consultar cobrança
    PAGAR: string      // pagamento pix
    PIX: string        // leituras gerais
  }
}

export function resolveInterEnv(): InterResolved {
  const env = (process.env.INTER_ENV ?? 'homolog') as 'homolog' | 'prod'
  ...
  const abs = (p?: string) => (p ? path.resolve(process.cwd(), p) : undefined)
