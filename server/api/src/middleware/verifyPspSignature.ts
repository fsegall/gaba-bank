...
export function verifyPspSignature(req: Request, res: Response, next: NextFunction) {
  ...
  const expected = crypto.createHmac('sha256', secret).update(`${ts}.`).update((req as any).rawBody || '').digest('hex')
