...
async function main() {
  ...
  try {
    ...
    await c.query(
      `insert into users (id, email, cpf, kyc_status, risk_tier)
       values ($1,'dev@defy.local','000.000.000-00','approved','standard')
       on conflict (id) do update set kyc_status='approved', risk_tier='standard'`,
      [USER_ID]
    )
    ...
    await c.query(
      `insert into wallets (user_id, asset, balance)
       values ($1,'BRL',0)
       on conflict (user_id, asset) do update set balance=0, updated_at=now()`,
      [USER_ID]
    )
