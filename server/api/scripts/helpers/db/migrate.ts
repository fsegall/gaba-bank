...
function resolveSqlDir(): string {
  ...
  throw new Error(
    `ENOENT: diretório de migrations não encontrado. Tente criar scripts/helpers/sql ou defina SQL_DIR.\n` +
...
async function withAdvisoryLock<T>(pool: Pool, fn: () => Promise<T>) {
...
async function applyFile(pool: Pool, filePath: string) {
  ...
  if (!existing) {
    ...
    throw new Error(
      `Checksum mismatch em ${filename}. O arquivo foi alterado após aplicado. Crie uma nova migration de correção.`,
    );
  } else {
