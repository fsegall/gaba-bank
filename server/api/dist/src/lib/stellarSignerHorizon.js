...
export async function signWithCLI(xdrB64, alias = "defy-treasury") {
    return new Promise((resolve, reject) => {
        const child = execFile("stellar", [
            "tx",
            "sign",
            "--sign-with-key",
            alias,
...
export async function submitSignedXDR(xdrSignedB64) {
    const url = `${HORIZON_URL.replace(/\/$/, "")}/transactions`;
    const response = await axios.post(url, `tx=${encodeURIComponent(xdrSignedB64)}`, { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
