import { DefindexSDK, SupportedNetworks,
...
const network = netEnv === 'mainnet' ? SupportedNetworks.MAINNET : SupportedNetworks.TESTNET;
...
async function readSharesAndUnderlying(addr) {
...
export class DefindexVaultProvider {
    kind = 'defindex';
    /**
    ...
    async deposit(args) {
        ...
        // 3) assinar e enviar
        const signedXdr = await signWithCLI(xdr, netEnv);
        const sent = await sdk.sendTransaction(signedXdr, network, false);
        const externalId = String(sent.hash || sent.transactionHash || sent.id); // tx hash como external_id
        // 4) leitura pós-envio
    ...
    async withdraw(args) {
        ...
        const signedXdr = await signWithCLI(xdr, netEnv);
        const sent = await sdk.sendTransaction(signedXdr, network, false);
        const externalId = String(sent.hash || sent.transactionHash || sent.id);
        const after = await readSharesAndUnderlying(treasury);
