...
function sanitizePayer(p) {
    if (!p)
        return undefined;
    const out = { nome: p.nome.trim() };
    if (p.cpf)
        out.cpf = p.cpf.replace(/\D/g, '');
    if (p.cnpj)
        out.cnpj = p.cnpj.replace(/\D/g, '');
    return out;
}
function isDuplicateTxidError(err) {
    const data = err?.response?.data;
    const viol = Array.isArray(data?.violacoes) ? data.violacoes : [];
    if (viol.some((v) => /txid/i.test(String(v?.propriedade)) && /utilizad/i.test(String(v?.razao))))
        return true;
...
export async function createPixCharge({ txid, amount, payer }) {
    ...
    if (!key)
        throw new Error('INTER_PIX_KEY não configurada');
    const body = {
...
// === Pagamento (Banking v2) ===
// src/providers/inter/client.ts
export async function payPix({ idempotency, // <-- novo nome, sem confundir com txid de COB
chave, amount, descricao, }) {
    if (!idempotency)
        throw new Error('idempotency é obrigatório');
    assertAmount(amount);
    const http = interHttp();
    const body = {
