...
const cache = new Map(); // cache por escopo
const expand = (s) => s?.replace(/\$\{([A-Z0-9_]+)\}/gi, (_, k) => process.env[k] ?? '');
const read = (p) => {
    if (!p)
        return undefined;
...
function resolveCreds() {
    ...
    if (!tokenURL)
        throw new Error('INTER_TOKEN_URL não configurado');
...
function resolveScope(name) {
    if (name === 'cob')
        return process.env.INTER_OAUTH_SCOPE_COB ?? 'pix.read pix.write';
    if (name === 'pix')
        return process.env.INTER_OAUTH_SCOPE_PIX ?? 'pix.read';
    if (name === 'pay')
        return process.env.INTER_OAUTH_SCOPE_PAGAR ?? 'pagamento-pix.write';
    return name || (process.env.INTER_OAUTH_SCOPE_COB ?? 'pix.read pix.write');
}
export async function getInterToken(scopeName) {
    const scope = resolveScope(scopeName);
    const now = Math.floor(Date.now() / 1000);
    const hit = cache.get(scope);
    if (hit && hit.exp - 10 > now)
        return hit.token;
