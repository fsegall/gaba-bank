...
function resolveCreds() {
    ...
    if (!baseURL)
        throw new Error('INTER_BASE_URL não configurado');
    if (!tokenURL)
...
async function fetchToken(agent, scopeRaw) {
...
function makeAgent() {
    ...
    if (!cert || !key || !ca)
        throw new Error('Certificados CRT/KEY/CA não configurados');
    return new https.Agent({ cert, key, ca, keepAlive: true, minVersion: 'TLSv1.2' });
}
export function interHttp() {
    if (cached)
        return cached;
    ...
    http.interceptors.request.use(async (cfg) => {
        ...
        cfg.headers['Authorization'] = `Bearer ${tok.token}`;
        return cfg;
    });
