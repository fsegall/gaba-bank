...
import { fromUnits } from "../money.js";
...
// helpers
const isClassic = (a) => a.includes(":") && !a.startsWith("C");
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            ...
            if (status && status < 500 && status !== 429)
                break; // 4xx (exceto 429) não re-tenta
            await new Promise(r => setTimeout(r, 250 * (2 ** i))); // 250ms, 500ms, 1s
        }
    }
    throw lastErr ?? new Error(`${tag}: failed`);
}
async function quoteClassic({ assetIn, assetOut, amountIn, // on-chain units (10^ONCHAIN_DEC)
slippageBps = 50, // default 0.50%
    ...
    const resp = await horizon.get("/liquidity_pools", { params });
    const pool = resp.data?._embedded?.records?.[0];
    if (!pool)
        throw new QuoteError("NO_POOL", { params, resp: resp.data });
    // 2) Mapear reservas na mesma ordem dos assets solicitados
    const rIn = pool.reserves.find((r) => r.asset === assetIn);
    const rOut = pool.reserves.find((r) => r.asset === assetOut);
    if (!rIn || !rOut)
        throw new QuoteError("POOL_RESERVES_MISMATCH", { pool });
    const X = Number(rIn.amount); // reserva do token de entrada
    const Y = Number(rOut.amount); // reserva do token de saída
    if (!(X > 0 && Y > 0))
        throw new QuoteError("EMPTY_RESERVES", { X, Y });
    ...
    const qLike = {
        ...
        tradeType: "EXACT_IN",
        platform: "sdex", // sinaliza que veio da Classic AMM
        routePlan: [{
                platform: "sdex",
                poolId: pool.id,
                fee_bp: 30,
...
export async function getQuote(req) {
    return withRetry(async () => {
        ...
        if (status >= 400)
            throw new QuoteError("Erro ao obter quote", data);
        // normaliza números vindos como number -> string
        if (typeof data?.amountOut === "number")
            data.amountOut = String(data.amountOut);
        if (typeof data?.amountIn === "number")
            data.amountIn = String(data.amountIn);
        if (typeof data?.otherAmountThreshold === "number")
            data.otherAmountThreshold = String(data.otherAmountThreshold);
        return data;
    }, "SOROSWAP_QUOTE");
}
export async function buildSwapTx(quote, from = TREASURY_PUB, to = TREASURY_PUB) {
    return withRetry(async () => {
        const { data, status } = await getHttp().post(`/quote/build`, { quote, from, to }, { params: { network: NETWORK } });
        if (status >= 400)
            throw new BuildError("Erro ao buildar swap", data);
        return data;
    }, "SOROSWAP_BUILD");
}
export async function signAndSubmit(xdrB64) {
...
export const soroswap = {
    async quote({ pair, amountIn }) {
        ...
        if (!assetIn)
            throw new QuoteError(`Asset env não definido: ${envIn}`);
        if (!assetOut)
            throw new QuoteError(`Asset env não definido: ${envOut}`);
        ...
        if (isClassic(assetIn) && isClassic(assetOut)) {
            ...
            try {
                ...
                if (Number(outHuman) > 0)
                    poolMid = Number(brlHuman) / Number(outHuman);
            }
            catch { /* noop */ }
            return {
        ...
        if (!q.amountOut)
            throw new QuoteError("amountOut ausente na quote", q);
        let poolMid = 0;
        try {
            const brlHuman = fromUnits(codeIn, amountIn);
    ...
    async swap({ route, amountIn, minOut }) {
        try {
            const q = route;
            ...
            const xdr = built.xdr || built.transaction_xdr;
            if (!xdr)
                throw new BuildError("Swap build não retornou XDR", built);
            const res = await signAndSubmit(xdr);
            return {
                exec_provider: "soroswap",
                route: q.routePlan ?? q,
                fee_native: res.fee_charged ?? null,
                tx_hash: res.hash ?? null,
            };
        }
        catch (e) {
            if (e?.name === "BUILD_ERROR" || e?.name === "SUBMIT_ERROR")
                throw e;
