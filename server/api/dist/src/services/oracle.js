...
const REFLECTOR_ID = process.env.REFLECTOR_CONTRACT_ID ??
...
const VIEW_SOURCE = process.env.SOROBAN_VIEW_SOURCE ??
    (isTest ? 'GDKMJY2VWJXQAF6Y2WJ5QXFU7YQ4ZMQAWO4S3J5EZ2ZXEGQXNAVKJZ6E' : undefined);
if (!RPC_URL || !REFLECTOR_ID || !PASSPHRASE || !VIEW_SOURCE) {
    throw new Error(`[oracle] missing envs. Set SOROBAN_RPC_URL, REFLECTOR_CONTRACT_ID, ` +
...
async function view(method, ...args) {
    ...
    const scv = xdr.ScVal.fromXDR(sim.result.retval);
    return scValToNative(scv);
}
function assetOther(symbol) {
...
export async function getUSDCperBRL_fromReflector() {
    const base = await view('base'); // SEP-40 base()
    const decimals = Number(await view('decimals'));
    const pd = await view('lastprice', assetOther('BRL'));
    const brlInBase = toFloat(BigInt(String(pd.price)), decimals); // base por 1 BRL
    // Se base é USD ou USDC, então USDC/BRL = 1 / (base/BRL)
    const isUsdBase = base.Other === 'USD';
    const isUsdcBase = !!base.Stellar || base.Other === 'USDC';
...
export async function getUSDCperBRLD_parity() {
...
export function __setOracleDepsForTests(d) {
    Object.assign(__deps, d);
}
/** Circuit breaker comparando BRLD→USDC com oracle (paridade BRL) */
export async function sanityCheckBRLDUSDC(routePrice) {
    if (process.env.ORACLE_SANITY_ENABLED !== 'true')
...
export async function sanityCheckBRLUSDC(routePrice) {
    if (process.env.ORACLE_SANITY_ENABLED !== 'true')
        return { ok: true, reason: 'disabled' };
