...
import { resolveEndpoints, sep10_start, sep10_token, sep38_quote, sep24_deposit, sep12_putCustomer, sep12_getCustomer, } from "../services/anchor.js";
export const router = Router();
const normalizeJwt = (v) => {
    const s = (v ?? "").trim();
    if (!s)
        return undefined;
...
const getJwt = (req) => normalizeJwt(req.body?.jwt) ||
    normalizeJwt(/^Bearer\s+(.+)$/i.exec(req.header("authorization") || "")?.[1]) ||
    normalizeJwt(req.query.jwt);
...
const wrap = (fn) => (req, res, next) => fn(req, res, next).catch((err) => {
    ...
    const body = typeof err?.response?.data === "object" ? err.response.data : undefined;
    res.status(status).json(body ?? { error: err?.message || "anchor_proxy_error" });
});
/** exige presença de JWT (SEP-10) — usado nas rotas SEP-12 */
const requireJwtSep10 = (req, res, next) => {
    const jwt = getJwt(req);
    if (!jwt)
        return badReq(res, "missing_jwt");
    req.__jwt = jwt;
    next();
};
/** --- Descobrir/forçar endpoints --- */
router.get("/anchor/toml", wrap(async (_req, res) => {
    const ep = await resolveEndpoints();
    res.json(ep);
}));
/** --- SEP-10: start + finish --- */
router.post("/anchor/sep10", wrap(async (req, res) => {
...
router.post("/anchor/sep24/deposit", wrap(async (req, res) => {
    const jwt = getJwt(req);
    if (!jwt)
        return badReq(res, "missing_jwt");
    const { asset_code, account } = req.body || {};
    if (!asset_code)
        return badReq(res, "missing_asset_code");
    if (!account)
        return badReq(res, "missing_account");
    const d = await sep24_deposit(jwt, asset_code, account);
    res.json(d); // { id, url }
}));
/** --- SEP-12: cria/atualiza cliente (AGORA COM MIDDLEWARE) --- */
router.post("/anchor/sep12/customer", requireJwtSep10, wrap(async (req, res) => {
    const jwt = req.__jwt;
...
router.get("/anchor/sep12/customer", requireJwtSep10, wrap(async (req, res) => {
    const jwt = req.__jwt;
...
/** --- SEP-24 /info (proxy) --- */
router.get("/anchor/sep24/info", wrap(async (_req, res) => {
    const ep = await resolveEndpoints();
