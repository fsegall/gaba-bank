...
router.get('/portfolio', async (req, res) => {
    ...
    const { rows: wallets } = await pool.query(`select asset,
            trim(trailing '.' from to_char(balance, 'FM999999999999990.9999999999')) as balance
       from wallets
      where user_id = $1
      order by asset`, [userId]);
    const { rows: orders } = await pool.query(`select id, product_id, side, symbol, amount_brl_centavos, status, created_at
       from orders
      where user_id = $1
