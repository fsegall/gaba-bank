...
import { Address, BASE_FEE, Contract, TransactionBuilder, nativeToScVal, xdr, } from "@stellar/stellar-sdk";
export function scvAddress(g) {
    const scAddr = Address.fromString(g).toScAddress();
    return xdr.ScVal.scvAddress(scAddr);
}
export function scvI128(v) {
    return nativeToScVal(v.toString(), { type: "i128" });
}
export async function sendAndWait(server, tx) {
    const send = await server.sendTransaction(tx);
    if (send.errorResult)
        throw new Error(`send error: ${send.errorResult}`);
