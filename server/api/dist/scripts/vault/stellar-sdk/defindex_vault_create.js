...
const NET = (process.env.DEFINDEX_NETWORK || 'testnet').toLowerCase() === 'mainnet'
...
async function signAndSend(xdr, secret) {
...
async function main() {
    ...
    if (!health?.status?.reachable)
        throw new Error('Defindex API unreachable');
    ...
    if (!create || !create.xdr)
        throw new Error('createVault returned null XDR after retries');
    ...
    const txHash = sent.hash || sent.transactionHash || sent.id;
    ...
    if (process.env.DEFINDEX_TREASURY_SECRET) {
        ...
        const amounts = [2000000]; // ex.: 2.0000000 unidades (7 dps)
        ...
        const depTxHash = depRes.hash || depRes.transactionHash || depRes.id;
